...
<div class="comtainer">
		<div class="row">
			<div class="col-lg-6">
				<span v-for="location in locations">
					<ul>
						<li><h3>{{location.location_name}}</h3></li>
					</ul>
				</span>
			</div>
		</div>
		...
		<!-- <vue-form ref="form" :state="formstate"  @submit.prevent="onSubmit">
			<div class="scroll_div" v-slimscroll="scrollOptions">
						...
						<div class="flex-td flex-wrap">
							<div class="multi-th">
								...
								<div class="td_time">
									...
									<h3>{{$global.convert_time(model.start_time)}} - {{$global.convert_time(model.end_time)}}, {{model.date}}</h3>
								</div>
								<div class="td_target">
				...
				<button class="btn btn-dark" @click="counter_func" type="button">{{$store.state.language.add_customer_popup.add_another_friend}}</button>
			</div>
